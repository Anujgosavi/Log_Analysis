...
exports.normalProductService = async (req, res) => {
  try {

    const min = 323 - 100;
    const max = 323 + 100;

    const responseTime = Math.floor(Math.random() * (max - min + 1)) + min;
    await new Promise((resolve) => setTimeout(resolve, responseTime));
    ...
    const logData = {
      ...
      status_code: 200,
      endpoint: "/products",
      response_size_bytes: 25564,
      request_size_bytes: 10287,
      client_ip: req.ip,
      http_method: "GET",
      service: "normal",
      geolocation: ['US', 'IN', 'DE'][Math.floor(Math.random() * 3)]
...
exports.slowProductService = async (req, res) => {
  try {
    const responseTime = Math.floor(Math.random() * (950-650)) + 650;
    await new Promise((resolve) => setTimeout(resolve, responseTime));
...
exports.fastProductService = async (req, res) => {
  try {
    const responseTime = Math.floor(Math.random() * 100);
    await new Promise((resolve) => setTimeout(resolve, responseTime));
