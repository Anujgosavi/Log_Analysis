...
const Anomaly = require("../models/Anomaly"); // add this import
      ...
      this.changeStream.on("change", async (change) => {
        if (change.operationType === "insert") {
          ...
          const docToSave = {
            ...
            geo_location_enc:
              parsedLog.geolocation_enc ?? parsedLog.geo_location_enc ?? -1,
            ...
            try {
              ...
              if (body.is_anomaly === true) {
                try {
                  ...
                  const savedAnomaly = await Anomaly.create(anomalyData);
                  console.log("⚠️ Anomaly saved with id:", savedAnomaly._id);
                  ...
                  // 3️⃣ Decode endpoint
                  const endpoint =
                    endpointMap[anomalyData.endpoint_enc] || "Unknown Endpoint";

                  console.log("Decoded endpoint:", endpoint);

                  try {
                    const slackMessage = {
                      text: `🚨 Response time too high for ${endpoint}. The response time is ${
                        1 / Math.log(anomalyData.log_response_time)
                  ...
                  console.error("❌ Error saving anomaly:", anomalyErr.message);
                }
              }
            } catch (apiErr) {
              console.error(
